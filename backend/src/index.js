...
// Add this temporary test endpoint after your other routes
app.get('/api/test-smtp-status', async (req, res) => {
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: 'snaq ptsl yqpm hufr'  // Your app password with spaces
      },
      tls: {
    ...
    const info = await transporter.sendMail({
      ...
      subject: 'SMTP Test - Render Server',
      html: `
        <h1>✅ SMTP is Working!</h1>
        <p>This email was sent from your Render server.</p>
        <p>Time: ${new Date().toLocaleString()}</p>
        <p>Your OTP emails will now work correctly.</p>
      `
    });
    
    res.json({
      success: true,
      message: 'SMTP is working! Test email sent',
